...
class Bill extends Model
{
    ...
    public function getBillImage($config=[])
    {
        ...
        try{
            if ($config['type'] ===1){
                // 添加二维码
                $this->addPic($backImg,$codeImg,110,110,535,710,$new);
                // 添加产品描述，对描述进行分行
//                $theTitle = $this->cn_row_substr($config['title'],3,11);
                $theTitle = explode("\n",$config['title']);;
                $yArr = [150,215,280,345,410,475,540];
                foreach ($theTitle as $k => $v){
                    $this->addWord($theTitle[$k],65,$yArr[$k],40,'black',$new);
                }
                $this->addWord($author,460,600,18,'black',$new);
                $this->addWord($notice,270,780,18,'black',$new);
                return $new;
            }
        }catch (\Exception $e){
...
//        $fontfile = FONT_PATH.'simkai.ttf';
        $fontfile = FONT_PATH.'YanSong.otf';
        //$str = mb_convert_encoding($str, "html-entities", "utf-8");
        imagettftext($s_original, $font, $tilt, $posX, $poxY, $ImgColor[$color], $fontfile, $str);
//        imagettftext($s_original, $font, $tilt, ($posX+1), $poxY, $ImgColor[$color], $fontfile, $str);
